...
export default {
  ...
  async mounted() {
    const userVariables = await invoke("get_user_variables", {});
    userVariables.forEach((pathData, index) => {
      let data = {};
      data.path = pathData;
  ...
  methods: {
    ...
    async save() {
      const userPath = this.inputEditPaths.join(";").trim();
      let saveResult = await invoke("save_user_variables", { userPath });
      if(saveResult) {
        this.emitter.emit('showToastEvent', { type: 'success', message: 'Saved user-path!'});
      }
      else {
        this.emitter.emit('showToastEvent', { type: 'error', message: 'Failed to save user-path!'});
      }
    }
  }
}
